...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('help')
    .setDescription('Show the commands available in this bot.'),
  new SlashCommandBuilder()
    .setName('server')
...
async function connectToMemberChannel(member) {
  ...
  if (existingSession && existingSession.channelId === member.voice.channel.id) {
...
client.on('messageCreate', async (message) => {
  ...
  try {
    ...
    if (command === '!d') {
      await message.reply(
        disconnectFromGuild(message.guild.id) ? 'Disconnected.' : 'I am not in a voice channel.',
      );
      return;
    }
